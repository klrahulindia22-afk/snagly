...
function Section({ title, children }) {
  return (
    <div style={{ background:"var(--modal-bg)", border:"1px solid var(--border)", borderRadius:16, padding:24 }}>
...
export default function ProfilePage() {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      setPwErr(err.response?.data?.detail?.message || err.response?.data?.detail || "Password change failed. Check your current password and try again.");
    } finally {
      setPwSaving(false);
    }
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--app-bg)" }}>
      {/* Top nav bar */}
      <div style={{ borderBottom:"1px solid var(--border)", padding:"12px 24px", display:"flex", alignItems:"center", gap:12, background:"var(--modal-bg)" }}>
        <button
          onClick={() => navigate(-1)}
          style={{ background:"none", border:"none", color:"var(--text-muted)", cursor:"pointer", fontSize:12, display:"flex", alignItems:"center", gap:4, fontFamily:"inherit" }}
        ...
        <Section title="Identity">
            ...
            <div style={{ position:"relative" }}>
              ...
              <label
                ...
                style={{
                  ...
                  background:"#6c63ff", color:"#fff", border:"none", cursor: avatarUploading ? "not-allowed" : "pointer", fontSize:12,
                  display:"flex", alignItems:"center", justifyContent:"center", opacity:avatarUploading?0.5:1,
                }}
              ...
              <input id="avatar-upload-profile" ref={fileRef} type="file" accept="image/*" style={{ display:"none" }} onChange={handleAvatarChange} />
            </div>
            <div>
          ...
          <form onSubmit={handleProfileSave} style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {COLOR_PRESETS.map((c) => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: c,
                      outline: color === c ? "3px solid var(--text-primary)" : "2px solid transparent",
                      outlineOffset:2,
            ...
            <button
              type="submit"
              disabled={saving}
              style={{ alignSelf:"flex-start", padding:"8px 16px", borderRadius:8, background:"#6c63ff", color:"#fff", border:"none", fontSize:13, fontWeight:500, cursor:"pointer", fontFamily:"inherit", opacity:saving?0.5:1 }}
          ...
          <form onSubmit={handlePasswordChange} style={{ display:"flex", flexDirection:"column", gap:10 }}>
            ...
            <div>
              ...
              <input
                ...
                style={{
                  ...inputStyle,
                  borderColor: confirmPw && confirmPw !== newPw ? "#de350b" : "var(--border)",
                }}
              ...
              {confirmPw && confirmPw !== newPw && (
                <p style={{ color:"#de350b", fontSize:12, margin:"4px 0 0" }}>Passwords do not match.</p>
              )}
            </div>
        ...
        <Section title="Two-factor authentication">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{
              ...
              background: user?.two_fa_enabled ? "rgba(97,189,79,0.12)" : "var(--input-bg)",
              color: user?.two_fa_enabled ? "#61bd4f" : "var(--text-muted)",
            }}>
          ...
          {tfaErr && (
            <div style={{ display:"flex", alignItems:"flex-start", gap:8, background:"rgba(222,53,11,0.08)", border:"1px solid rgba(222,53,11,0.25)", borderRadius:8, padding:"8px 12px" }}>
              ...
              <p style={{ color:"#de350b", fontSize:12, margin:0 }}>{tfaErr}</p>
            </div>
          )}
              ...
              <div style={{ background:"var(--input-bg)", borderRadius:10, padding:"12px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
                <div>
                  ...
                  <p style={{ fontSize:12, color:(user?.backup_codes_remaining ?? 0) <= 2 ? "#de350b" : "var(--text-muted)", margin:0 }}>
                    {user?.backup_codes_remaining ?? 0} of 10 codes remaining
                    {(user?.backup_codes_remaining ?? 0) <= 2 && " — regenerate soon"}
                  </p>
                </div>
                <button
                  onClick={() => { setShowRegenPanel((v) => !v); setRegenCodes(null); setRegenErr(""); setRegenTotp(""); }}
                  ...
                  {regenCodes ? (
                    <>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"4px 24px", marginBottom:12 }}>
                        {regenCodes.map((c, i) => (
                          <span key={i} style={{ fontFamily:"monospace", fontSize:13, color:"var(--text-primary)", letterSpacing:1 }}>{c}</span>
                        ))}
                      </div>
                      <button
                        onClick={() => {
                    ...
                    <>
                      <p style={{ fontSize:13, color:"var(--text-primary)", margin:"0 0 10px" }}>
                        Enter your 6-digit authenticator code to generate 10 new static backup codes. Your old backup codes will stop working immediately.
                      </p>
