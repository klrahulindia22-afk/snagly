...
const S = {
  ...
  step:  { fontSize:11, fontWeight:700, color:'#0f9e8e', textTransform:'uppercase', letterSpacing:.5, marginBottom:10 },
  ...
  btn:   { background:'#0f9e8e', color:'#fff', borderRadius:4, height:40, padding:'0 16px', fontSize:14, fontWeight:600, width:'100%', display:'flex', alignItems:'center', justifyContent:'center', border:'none', cursor:'pointer', fontFamily:F, marginTop:8 },
...
export default function Setup2FA() {
  ...
  useEffect(() => {
    setup2fa()
      .then(data => { setUri(data.totp_uri); setSecret(data.secret); setStep('qr') })
      .catch(ex => {
