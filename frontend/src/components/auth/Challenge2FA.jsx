...
const S = {
  ...
  mark:  { fontSize:22, fontWeight:800, color:'#0f9e8e', letterSpacing:-1, display:'flex', alignItems:'center', justifyContent:'center', gap:8 },
  ...
  btn:   { background:'#0f9e8e', color:'#fff', borderRadius:4, height:40, padding:'0 16px', fontSize:14, fontWeight:600, width:'100%', display:'flex', alignItems:'center', justifyContent:'center', border:'none', cursor:'pointer', fontFamily:F, marginTop:4 },
...
export default function Challenge2FA() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const data = await login2fa(email, tab, code.trim())
        ...
        <div style={S.tabs}>
          {TABS.map((t, i) => (
            <button key={t.id} onClick={() => resetState(t.id)} style={{
              ...
              background: tab === t.id ? '#0f9e8e' : '#f4f5f7',
              ...
              {!otpSent ? (
                ...
                <>
                  ...
                  <button type="button" onClick={handleSendEmailOtp}
                    ...
                    style={{ background:'none', border:'none', cursor: otpCountdown > 0 ? 'not-allowed' : 'pointer', color: otpCountdown > 0 ? '#8993a4' : '#0f9e8e', fontFamily:F, fontSize:12, padding:0, marginBottom:14 }}>
